...
const Sidebar: React.FC<SidebarProps> = ({ sidebarOpen }) => {
  ...
  const isManagerRoute = pathname.startsWith('/manager'); // Determine if it's a manager route

  const menuItems: MenuItem[] = isAgentRoute
  ? [
      { name: 'Sales', icon: 'dollar-sign', href: '/$', disabled: true }, // Add disabled property
      { name: 'Mails', icon: 'mail', href: '/$', disabled: true }, // Add disabled property
    ]
  : isManagerRoute
  ? [
      // Menu items specific to the manager route
      { name: 'Dashboard', icon: 'home', href: '/dashboard' },
      { name: 'Agent', icon: 'users', href: '/accessagents' },
      { name: 'Query', icon: 'edit', href: '/query' },
    ]
  : [
      // Default menu items when it's not an agent or manager route
      { name: 'Dashboard', icon: 'home', href: '/dashboard' },
      { name: 'Agent', icon: 'users', href: '/accessagents' },
      { name: 'Manager', icon: 'users', href: '/accessmanagers' },
      { name: 'Query', icon: 'edit', href: '/query' },
    ];
  ...
  {menuItems.map((item, index) => (
    <li
      key={index}
      className={`flex items-center pl-4 py-2 group cursor-pointer hover:bg-[#232a2f] transition-colors duration-300
        ${item.disabled ? ' cursor-not-allowed' : 'text-[#B3B3B3] hover:text-white'}
      `}
      onClick={(e) => {
        if (item.disabled) {
          e.preventDefault(); // Prevent click if the item is disabled
        }
      }}
    >
      ...
      {item.disabled ? (
        <span>{item.name}</span> // No Link component for disabled items
      ) : (
