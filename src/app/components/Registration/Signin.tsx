...
function Signin() {
  ...
  const onSubmit = async (data: { role: string; email: string; password: string; }) => {
    ...
    try {
      ...
      switch (data.role) {
        case 'admin':
          endpoint = 'https://crm-syste-f15f21fd2637.herokuapp.com/api/login/admin';
          break;
        case 'agent':
          endpoint = 'https://crm-syste-f15f21fd2637.herokuapp.com/api/login/agent';
          break;
        case 'manager':
          endpoint = 'https://crm-syste-f15f21fd2637.herokuapp.com/api/login/manager';
          break;
        default:
      ...
      if (response.ok) {
        ...
        if (data.role === 'admin') {
          ...
          router.push(`/managerdashboard/${id}`); // Redirect to manager page
        }
      } else {
        toast.error('Invalid credentials or user not active');
      }
    } catch (error) {
        ...
        <div className="grid md:grid-cols-10 min-h-screen text-white">
          ...
          <div className="col-span-5 from-[#0d1137] bg-registration-mobile to-[#020515] 2xl:p-0 p-5 bg-gradient-to-b flex flex-col justify-center items-center gap-4">
            ...
            <div className="text-center plus-jakarta-sans-400 mt-9">
              ...
              <span className="text-[#a0aec0] text-sm font-normal leading-[21px]"><strong>Infinity waves</strong></span>
